...
class TL_Inverse(Topo):
    def build(self):
        ...
        self.add_node("4")
        self.add_node("5")
        self.add_link_name("0", "3", cost=1000, delay=0.2, bw=40000, directed=True)
        self.add_link_name("3", "0", cost=1000, delay=0.2, bw=40000, directed=True)
    ...
    def dijkstra_computed(self):
        # Demand from 0 to 4
        for path, frac in []:
            build_str = f"ip -6 route add {dst} metric 1 table 1 src {src}"
            for u, v in zip(path[:-1], path[1:]):
                nhlist = self.get_dijkstra_route_by_name(str(u), str(v))
                for nh in nhlist:
                    build_str += f" nexthop via {nh.nh}"
            weight = int(frac * 100)
            build_str += f" weight {weight}"
            self.add_command(str(src), build_str)
        # Demand from 0 to 5
        for path, frac in []:
            build_str = f"ip -6 route add {dst} metric 1 table 1 src {src}"
            for u, v in zip(path[:-1], path[1:]):
...
topos = {'TL_Inverse': (lambda: TL_Inverse())}
