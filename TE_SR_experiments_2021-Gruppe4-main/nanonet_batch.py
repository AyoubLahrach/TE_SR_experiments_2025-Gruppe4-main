...
class Tests:
	def __init__(self):
		...
		#self.TESTFILES = ['TL_Inverse.topo.sh', 'New_Weights.topo.sh']
		#self.JSON_FILES = ['tl_inverse.json', 'new_weights.json']
		self.TESTFILES = ['Dfw.topo.sh', 'Weights_2.topo.sh']
		self.JSON_FILES = ['dfw.json', 'weights_2.json']
	...
	def run_test_case(self, test_name : str):
		if not self.is_in_test:
			...
			exit(1)
		# 1 Start scripts
		print("Start script "+test_name+" (synchronous)")
		# Führe das Topo-Skript direkt aus und warte, bis es durchgelaufen ist
		p = run(['sudo', 'bash', f'./{test_name}'],
			stdout=PIPE,
			stderr=PIPE,
			encoding='utf-8',
			errors='replace')
		print(f"Process test {self.TEST_ID} returned with {p.returncode}.")
		# Log-Ausgaben sichern
		with open(f"{self.TEST_ID}_{test_name}.log", "a") as logfile:
			logfile.write(p.stdout)
			logfile.write(p.stderr)
			
		# Warte TIME+Puffer, damit throughput.py -s komplett durchläuft (z.B. 5 min + 1 min)
		sleep(self.TIME + 60)
		print("Test should be finished … (synchronous)")
	...
	def finish_test_case(self, test_name : str):
		...
		p = run(['sudo','bash', './'+test_name, '--stop'],
				stdout=PIPE,
				stderr=PIPE)
